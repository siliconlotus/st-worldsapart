...
read the file, and read nothing else. Do not open the chat, the lorebook, or any other entry to settle
a call, and do not go looking for a file whose path did not resolve: report what was wrong with your
input instead. A grade reached from context the retriever did not have is not a grade of the retriever.

If the job carries an `out` path, write that JSON to it and reply with one line: the path and the
number of grades. Otherwise return only the JSON.
