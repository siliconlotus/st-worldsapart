...
// production would build, so a divergence produces plausible wrong numbers attributed to whatever parameter
// was being swept. The oracle at the bottom is the real guard — it rebuilds from a graded sample's embedded
// books at that sample's own settings and asserts the (hash, uid) multiset matches what ST actually wrote.
...
// Reference is excluded at build time because a memory-tier centroid carrying reference chunks would
// measure the register blend the arm exists to avoid.
...
// No global de-duplication, and this is the counter-intuitive one: the hash carries (text, uid), because ST
// core lists and deletes by hash only, so identity has to live IN the hash or one hash stands for two owners
// and syncs misattribute rows. Text shared by two entries therefore hashes differently per owner and is
// stored twice. Collapsing them changes which entry owns a shared chunk, and since entry pooling takes the
// max over an entry's chunks, that moves the ranking and the elbow cut (P4).
...
for (const file of existsSync(DATA) ? readdirSync(DATA).filter(f => f.endsWith('.json')) : []) {
    ...
    if (stored.length === mine.length && stored.every((k, i) => k === mine[i])) {
        ...
        // Extras can be legitimate: syncWorld filters against what is already saved, so a row retired from
        // the entries but not yet vacuumed can survive in the collection. Missing rows cannot be —
        // everything stored came from these entries at these settings, so a rebuild that fails to reproduce
        // a stored item means buildItems has drifted from syncWorld.
