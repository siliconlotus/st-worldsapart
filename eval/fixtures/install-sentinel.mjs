...
console.log('                            union check: core reads "?" as a literal and never matches it;');
...
console.log('                            WOULD match it; that divergence is the entry\'s whole point)');
...
console.log('   cold frame (sticky 3)    say "cold frame" once: activates, then stays in the prompt the');
...
console.log('   first light (cooldown 2) activates on mention, then refuses to re-activate for 2 messages');
...
console.log('   workshop → duskmoth      mention "workshop": uid 12 activates from chat and its content');
