...
The scene and the candidates are given to you inline, and they are everything you get. You cannot read
a file and must not try to reach for one: a grade reached from context the retriever did not have is
not a grade of the retriever. If something you were meant to be given is missing, say what is missing
rather than working around it.

If you are given an `out` path, write that JSON to it and reply with one line: the path and the number
of grades. Otherwise return only the JSON.
