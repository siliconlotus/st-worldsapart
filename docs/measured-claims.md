...
- **R1** — Removing the stage-1 admission gate was a no-op: `bm25 > 0` admitted 99.9% of every book's
  ...
  relevant entry (70 graded scenes). — `plugin/scoring.mjs:5`; `docs/matching.md:365`; `CLAUDE.md:249`.

- **R2** — A strict cosine gate would lose 110 of 672 graded-relevant entries: chunks below the corpus
  mean carrying the query's exact terms (70 scenes). — `plugin/scoring.mjs:20`; `docs/matching.md:372`.
...
- **R11** — Summarization screen, n=106 scenes paired, F2 over delivered set, one local summarizer at
  ...
  — `docs/matching.md:392,:402,:406,:410`.

- **R12** — `baselineQuery` (removed) measured harmful: 374-trial LOO grid, monotonic decline, no
  beneficial weight (`baseline-grid.mjs`); mean-centering measurably helps, +8.8% nDCG@5
  (`centering-grid.mjs`). Single-scene A/B agrees: nDCG@5 1.000→0.918 at w1.0, recall 9/9→8/9 (n=1).
  — `extension/state.mjs:79`; `eval/eval-data/README.md:50` (gitignored).
...
- **R13** — messageDepth dose-response, n=80 scenes paired vs each scene's own depth-10 capture:
  ...
  at 20 (−0.009, p=0.044). ~6k chars of query at 10. — `extension/state.mjs:163`.
...
- **R14** — Core comparison (`eval/core-compare.mjs`, this corpus): core nominates 34.2 memory entries
  ...
  28 at depth 10 (89 scenes). — `eval/core-compare.mjs:4,:11,:33,:38,:73`.

- **R18** — Per-book centring leaves book identity intact: 1-NN same-book purity 99.4%→98.2%.
  — `eval/lib/scene.mjs:528`; `eval/param-screen.mjs:178`.
...
- **R19** — Entity filter effect: unfiltered BM25 ran ~2x the filtered value on one real scene (101.06
  ...
  smaller than the original note claimed. — `worldsapart.js:427`; `extension/state.mjs:140`.
...
- **R20** — Gazetteer source no longer moves the population: n=71 scenes paired, all four arms flat
  ...
  36,789 — bodies ≈ 5–10x the vocabulary; the keys-live arm read 9839). Before the architecture change
  these arms moved admission by up to 137 rows. The `denseColumn` reweighting arms moved +0.0056 and
  +0.0005 nDCG@10 over the same 70 scenes — a question the vector weight asks directly. The old
  population effect ("adds 767 keyword-only rows, loses 174 vector rows, 65 scenes") was an
  admitCeiling-100 artifact [SUPERSEDED].
  — `extension/entity.mjs:38,:94,:106`; `eval/param-screen.mjs:70,:87,:93,:139`.
...
- **R21** — Entity-tuning trap run results (recorded so the metric choice survives): relevant-per-scene
  ...
  — `extension/entity.mjs:78,:87,:91`; `eval/graded-scene-grid.mjs:292,:394`.
...
- **R23** — Fusion evidence: rrfK=60 hurt all three scenes, rrfK=10 helped two (less flattening reads
  ...
  3.5 while text reaches 158). — `eval/fusion-grid.mjs:16,:60`.
...
- **R24** — Chunk settings: paragraph distribution over 3642 paragraphs of 604 memory entries — mean
  ...
  significant, 4 of 5 lineages positive). — `extension/state.mjs:13,:18,:21`.
...
- **E4** — Delivered count is a corpus property, not a model property: at cutoff 0.10 all seven models
  ...
  in matcher-design). — `embedding-models.md:53`; `extension/state.mjs:87`; `docs/matching.md:755`.
...
- **E5** — Cutoff sweep (shipped model): 0.05→26.8 delivered / 27.1% P / 83.9% R; 0.10→13.3/38.1/69.5;
  ...
  — `embedding-models.md:64,:72,:78`; `extension/state.mjs:97`.

- **E6** — At F2's own optimum jina delivers 30.2 entries vs Qwen3-8B's 15.0 and still scores lower — a
  looser dial, not a better model. — `embedding-models.md:93`.
...
- **E7** — Query instructions (same collections, only the query vector moves): Qwen3-8B's instruction
  ...
  pair is flat (0.7976 vs 0.7982) and gets neither. — `embedding-models.md:41`;
  `extension/relevance.mjs:191,:218`; `eval/lib/reindex.mjs:145`; `test/model-resolve-check.mjs:39`.
...
- **E13** — Fit transfer (2026-08-30, 74 bundles, jina and qwen8b as the extreme embedders):
  ...
  `docs/matching.md:659`; `extension/relevance.mjs:165` (mxbai beta +0.337, low-middle of seven).
...
- **E14** — Fits do not transfer across embedders: memory-tier cosine +0.3113 under bge-m3 vs +0.7460
  ...
  text +0.567 / keys +0.137 (6051 rows, 102 scenes). — `worldsapart.js:571`;
  `test/relevance-model-check.mjs:119`; `CLAUDE.md:273`.
...
- **F6** — The idf weighting is what makes it work: idf beats count 45 up / 14 down (p 0.0001);
  ...
  (p 0.0002). — `docs/matching.md:1110,:1107`; `extension/relevance.mjs:249`; `eval/relevance-regress.mjs:193`.

- **F7** — The `entity` extractor beats the private ASCII regex it replaced: F2 0.5443→0.5476 paired
  over 88 scenes (47/17/24, p 0.0002); validation-fold AP 0.873→0.883. — `docs/matching.md:1062`;
  `eval/relevance-regress.mjs:201`.
...
- **F8** — Multi-token spans add nothing: −0.0019 F2 (18/28/48, p 0.184), 2 of 5 books, AUC identical
  ...
  scored scenes. — `docs/matching.md:1070`.
...
- **F11** — The length/density lattice: `length` costs wherever it sits (F2 0.4801→0.4723 alone;
  ...
  0.7958 / 0.345 / 0.5070 at 20.0 delivered. — `docs/matching.md:1159,:1165,:1167,:1353`.

- **F13** — Story-time position (uid) carries nothing: −0.0055 mean F2, 24 up / 53 down (p 0.0013),
  AUC flat. — `docs/matching.md:1210`.
...
- **F14** — Polynomial and interaction terms measured worse: `--degree 2` costs AUC 0.7989→0.7966, AP
  ...
  — `docs/matching.md:1216,:1225,:1231`.
...
- **F16** — An ungraded row is not a negative: `--ungraded-negative` moves rows 6051→7088, prevalence
  ...
  27.6/14.3/8.8→26.0/13.6/8.6%, F2 0.5081→0.5021 delivering 18.9 vs 17.9. — `docs/matching.md:1243`.
...
- **F17** — AUC flatters rare bands: at grade ≥4 it reads 0.975 while 90% recall costs 17.5%
  ...
  AP 0.324→0.139 (the signals were finding the paraphrase). — `docs/matching.md:1251,:1525`.
...
- **F18** — Tier signal coverage: 99.8% of memory rows vectorized; 84% of reference rows keyword-only;
  ...
  — `docs/matching.md:1265,:1267`; `extension/relevance.mjs:28`.

- **F19** — `density` inverts on reference: −0.935 (SE 0.185) vs +0.215 on memory, solo AUC 0.336
  there. [D3: `worldsapart.js:561` still carries an older pair, +0.21/−0.58.]
  — `docs/matching.md:1204`; `extension/relevance.mjs:270`; `worldsapart.js:633,:561`.
...
- **F28** — Straddling entries are the haystack paraphrased: 66 of 446 memory positives straddle their
  ...
  books half once read precision 33.5% → 15.5% (`test/paired-check.mjs:270`).
  — `extension/relevance.mjs:41`; `eval/lib/scene.mjs:56,:79`; `test/relevance-model-check.mjs:192`; `extension/state.mjs:198`.
...
- **F29** — The scale is ordinal in the signals and the flat spot is memory's: cosine across the four
  ...
  (+0.123 / +0.588 / +1.397). — `docs/matching.md:1389,:1397`; `extension/relevance.mjs:13`.
...
- **F30** — Calibration: P(≥3) indistinguishable from calibrated (ECE 0.0077 vs 0.0067 null, p 0.252);
  ...
  — `docs/matching.md:1546,:1550`; `eval/lib/logistic.mjs:203`.

- **F31** — The probability clamp: 39 of 8975 rows invert P(≥3) > P(≥2), by at most 0.0002.
  — `docs/matching.md:1425`; `extension/relevance.mjs:299`; `eval/relevance-regress.mjs:1109`.

- **F32** — Grade-2 rows are a steady ~20% of what E[credit] surfaces at every depth (22.0% of top-5,
  19.4% of top-20) — they pay into precision, not recall. — `docs/matching.md:1430`.
...
- **F34** — Two cutoffs, one per tier: memory peaks at 0.08 (F2 0.5139, 17.5 vs 4.0 relevant),
  ...
  precision. — `docs/matching.md:1462,:1467`.
...
- **F35** — Reference cosine wants `denseAllEntries`: fitted only on entries that happen to carry one
  ...
  quantity does not exist. [D4/D5: also quoted as +0.806 with AUC 0.7387→0.7851 and log-loss
  0.5539→0.5163 against text's +0.419, and as 0.717.]
  — `docs/matching.md:1471,:1475,:1306,:1375,:1500`; `worldsapart.js:814,:563`.

- **F37** — Reference tolerates a weak fit: AUC 0.733→0.698 held out (vs memory's 0.820→0.799), on 342
  vs 6051 rows. — `docs/matching.md:1517`.
...
- **F38** — The tier base-rate argument was measured wrong: pooled the tiers look 3.7x apart [5x at
  ...
  — `docs/matching.md:1380,:1385,:1522`.
...
- **F39** — Tier base rates as the eval uses them: memory ~7%, reference ~30%; a threshold that read
  ...
  — `eval/lib/scene.mjs:991`; `test/paired-check.mjs:64`; `eval/lib/logistic.mjs:187`.
...
- **F40** — Keyword tilt dose-response is closed: 13 doses 0.75–3 over 70 scenes, unimodal on both
  ...
  ~−0.02 nDCG). — `eval/param-screen.mjs:194`.
...
- **F42** — The metric window is load-bearing: moving from top-10 to the admitted set roughly halved
  ...
  macro mean went up. — `eval/param-screen.mjs:284`; `eval/relevance-regress.mjs:1071`.
...
- **F43** — Fit-matrix statistics come from every candidate, not the graded subset: at 73% pool
  ...
  — `eval/relevance-regress.mjs:741`.

- **F44** — Centroid population (memory tier vs `vectorized`): measured flat — −0.0003 n@10, −0.0016
  F2, paired over 103 scenes [104 in `worldsapart.js:863` — D10] on 4 lineages; the two centroids sit
...
- **F55** — Pooled standardisation sits above per-scene on the WHOLE-SYSTEM cost curve, at matched
  ...
  measurably improved. — `docs/matching.md:1009`.
...
- **F56** — The gain is the REFERENCE tier's; memory is flat with a negative lean. Reference held-out
  ...
  are seven views of ONE corpus, not seven draws. — `docs/matching.md:1009`.
...
- **K6** — Unclosed preset tags are real: five `<internal_states>` opens across ten messages, no
  close, on the motivating chat. — `docs/matching.md:312`; `extension/matcher.mjs:249`.
...
- **K7** — PARAGRAPH_BREAK is blank-line, not single-newline: 27.6% of messages carry blank-line
  ...
  chats, 392 messages, 780KB). — `extension/matcher.mjs:227`.
...
- **S1** — The count instruction is the largest lever found: raising it moved agreement with the
  ...
  — `eval/count-sweep.mjs:1`; `eval/count-vs-temp.mjs:3,:10`.
...
- **S2** — The shipped count wording won on worst-case F over 5 wordings × 6 model configurations: 5
  ...
  the lowest-yield wording edges it by 0.006 inside the noise floor. — `extension/keyword-suggest.mjs:42,:47`.
...
- **S3** — Name capitalisation ratio separates cleanly: real names 99.5–100% vs junk 1.6–16.4%, "lord"
  ...
  n-gram, "aldric" escaped at 6.42 repeats. — `extension/keyword-suggest.mjs:189,:232`.
...
- **S9** — The cap: uncapped over 39 books / 3405 entries an entry yields median 17 / mean 27
  ...
  names covers 98.8% of entries. — `extension/keyword-suggest.mjs:692,:713`.
...
- **S15** — Embedding drift detection ruled out: seed-variant cosine reads surface overlap ~1.71x as
  ...
  threshold). — `docs/keyword-suggestions.md:224`.
...
- **S19** — Genre shapes over one user's 35 books: acronyms in 30, bracket tags 26, apostrophe names
  ...
  from one of these. — `eval/genre-cases.mjs:4`.
...
- **G1** — Bundle-corpus census (migrated, 2026-08): 598 human verdicts, 16,962 judge verdicts, 12,519
  ...
  verdicts in the wrong field; every one of the 107 frozen haystacks re-derives from its source chat
  [D12: counts unreconciled in-repo]. — `CLAUDE.md:136`; `bundle-schema.md:513`;
  `extension/grading.mjs:153`; `docs/matching.md:1564,:1572`.
...
- **G3** — Contract self-agreement (re-grade at original job size: 179 rows / 12 jobs / 4 books):
  ...
  side between passes of the same judge. — `CLAUDE.md:166`; `docs/matching.md:1533`;
  `eval/judge-agree.mjs:16`; `extension/grading.mjs:159`.
...
- **G5** — Anchored wording is the lever: restating the question — same rater, same entries — moved
  ...
  — `.claude/agents/scene-relevance.md:16` (byte-identical in the sr-* family and all 16
  `eval/contracts/*.md`); `extension/grading.mjs:1069`.
...
- **G6** — Judge calibration facts carried in the rubric: ~half of all graded rows are 0, under a
  ...
  discount cost more than it bought (two human-4 siblings split 2 and 3). — `.claude/agents/scene-relevance.md:96,:122,:47`.
...
- **G7** — A later pass is not a better pass: re-grading the same rows with the same model under a
  ...
  — `bundle-schema.md:512`; `extension/grading.mjs:145,:153`.
...
- **G8** — Schema decisions, each on a measured sweep: 44 book filenames already contain spaces,
  ...
  167 commits past the 1.17.0 its package.json's 1.18.0 implied. — `bundle-schema.md:23,:193,:217,
  :225,:236,:275,:296,:450,:461,:466,:475,:494,:526`; `extension/grading.mjs:32,:533,:630,:680,:851`.
...
- **G13** — Capture conventions verified against the corpus: all 97 pre-existing bundles record depth
  ...
  usable 5340, 7 hidden). — `eval/synth-scenes.mjs:87,:353,:14`; `eval/lib/scene.mjs:550`.
...
- **C7** — Disabled entries are not a weak-entry population: they are EARLIER (mean story position
  ...
  coherence and centroid distance all go to chance. — `eval/eval-data/README.md:124` (gitignored);
  `eval/relevance-regress.mjs:412`.
