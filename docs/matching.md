...
A `/pattern/flags` key is matched by JavaScript's own engine on the raw text. Everything here is true
of a whole-key regex and of a `/…/` term inside a SmartKey alike.
...
are Unicode-aware**: a word character is any letter, digit or combining mark in any script, so whole-word
